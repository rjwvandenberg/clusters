#!/bin/bash
VERSION="v0.13.1"
curl -L --fail --remote-name-all https://github.com/moby/buildkit/releases/download/$VERSION/buildkit-$VERSION.linux-$ARCH.tar.gz
sudo tar -C /usr/local --no-same-owner -xzvf buildkit-$VERSION.linux-$ARCH.tar.gz
rm buildkit-$VERSION.linux-$ARCH.tar.gz
